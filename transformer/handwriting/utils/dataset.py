...
from utils.model_utils import subsequent_mask
...
class HandwritingDataset(Dataset):
    ...
    def __init__(
        ...
        strokes = np.load(
            data_path + "strokes.npy", allow_pickle=True, encoding="bytes"
        )
        ...
        for i, (seq_len, text_len) in enumerate(zip(lengths, char_lens)):
            ...
            data[i, seq_len:, 0] = 1.0
            char_mask[i, :text_len] = 1.0
    ...
    def char_to_idx(self, char_seq):
        return np.array([self.char_to_id[char] for char in char_seq]).astype(np.long)
